...
class VDF
{
    ...
    {
        if(!is_string($text)) {
            trigger_error("decode expects parameter 1 to be a string, " . gettype($text) . " given.", E_USER_NOTICE);
            return NULL;
        }
        ...
        if(count($stack) !== 1)  {
            trigger_error("VDF::decode: open parentheses somewhere", E_USER_NOTICE);
            return NULL;
        }
    ...
    public static function encode($arr, $pretty = false)
    {
        if(!is_array($arr))
        {
            trigger_error("VDF::encode expects parameter 1 to be an array, " . gettype($arr) . " given.", E_USER_NOTICE);
            return NULL;
        }
